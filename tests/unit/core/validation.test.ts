...
describe('Validation', () => {
	describe('validate', () => {
		...
		it('should validate valid data', () => {
			expect(() =>
				validate({ name: 'John', age: 30, email: 'john@example.com' }, schema)
			).not.toThrow();
		});

		it('should reject missing required fields', () => {
			expect(() => validate({ age: 30 }, schema)).toThrow();
		});

		it('should reject invalid types', () => {
			expect(() => validate({ name: 123 }, schema)).toThrow();
		});

		it('should validate string constraints', () => {
			expect(() => validate({ name: '' }, schema)).toThrow();
		});

		it('should validate number constraints', () => {
			expect(() => validate({ name: 'John', age: 150 }, schema)).toThrow();
		});

		it('should validate patterns', () => {
			expect(() => validate({ name: 'John', email: 'invalid' }, schema)).toThrow();
		});
	...
	describe('CommonSchemas', () => {
		it('should validate document ID', () => {
			expect(() =>
				validate({ documentId: 'ABC123-DEF456' }, CommonSchemas.documentId)
			).not.toThrow();

			expect(() => validate({ documentId: 'invalid!' }, CommonSchemas.documentId)).toThrow();
		});

		it('should validate title', () => {
			expect(() => validate({ title: 'Valid Title' }, CommonSchemas.title)).not.toThrow();

			expect(() => validate({ title: '' }, CommonSchemas.title)).toThrow();
		});

		it('should validate path', () => {
			expect(() => validate({ path: '/valid/path' }, CommonSchemas.path)).not.toThrow();

			expect(() => validate({ path: '../traversal' }, CommonSchemas.path)).toThrow();
		});
	});
	...
	describe('validatePath', () => {
		// validatePath === sanitizePath: it normalizes and drops empty/'.'/'..'
		// segments and null bytes (traversal is neutralized by removal, not by throwing).
		it('normalizes a path and drops the leading separator', () => {
			expect(validatePath('/valid/path')).toBe('valid/path');
			expect(validatePath('C:\\Windows\\System32')).toBeTruthy();
		});

		it('neutralizes path traversal by removing .. segments', () => {
			expect(validatePath('../etc/passwd')).not.toContain('..');
			expect(validatePath('a/../../etc/passwd')).not.toContain('..');
		});

		it('strips null bytes', () => {
			expect(validatePath('file\0name')).not.toContain('\0');
		});
