...
jest.mock('../../../src/services/ai/hms-vector-store.js', () => ({
	LangChainHMSVectorStore: {
...
jest.mock('@langchain/core/documents', () => ({
	Document: jest.fn((args: any) => args),
}));
...
describe('Compilation Handlers', () => {
	...
	beforeEach(() => {
		mockProject = {
			...
			compileDocuments: jest.fn().mockResolvedValue('Fallback compiled content' as any),
			exportProject: jest
				.fn()
				.mockResolvedValue({ success: true, path: '/export/path' } as any),
	...
	describe('compileDocumentsHandler', () => {
		it('should compile documents with AI enhancement', async () => {
			...
			expect((result.content[0] as any).data.langChainProcessed).toBe(true);
		});
		...
		it('should filter documents by root folder', async () => {
			...
			expect((result.content[0] as any).data.documentCount).toBe(2);
		});

		it('should fallback on AI compilation failure', async () => {
			const { LangChainCompilationService } =
				await import('../../../src/services/compilation/langchain-compiler.js');
			...
			expect((result.content[0] as any).data.enhanced).toBe(false);
			expect((result.content[0] as any).data.fallbackReason).toBe('AI service unavailable');
		});
		...
		it('should handle empty document list', async () => {
			...
			expect((result.content[0] as any).data.documentCount).toBe(0);
		});
	});
	...
	describe('compileDocumentsHandler', () => {
		it('should perform intelligent compilation', async () => {
			...
			expect(result.content[0].text).toBe('Compiled with AI');
			expect((result.content[0] as any).data.enhanced).toBe(true);
			expect((result.content[0] as any).data.targetOptimization).toBe('agent-query');
			expect((result.content[0] as any).data.documentCount).toBe(1);
		});
			...
			jest.mocked(LangChainCompilationService).mockImplementationOnce(
				() =>
					({
						...
						compileWithAI: jest
							.fn()
							.mockRejectedValue(new Error('Service error') as any),
					}) as any
			);
	...
	describe('generateMarketingMaterialsHandler', () => {
		it('should generate marketing materials', async () => {
			...
			expect((result.content[0] as any).data.enhanced).toBe(true);
			expect((result.content[0] as any).data.materialType).toBe('synopsis');
			expect((result.content[0] as any).data.targetAudience).toBe('agents');
		});
		...
		it('should handle different material types', async () => {
			...
			for (const materialType of materialTypes) {
				...
				const result = await generateMarketingMaterialsHandler.handler(args, mockContext);
				expect((result.content[0] as any).data.materialType).toBe(materialType);
			}
		});
		...
		it('should handle different lengths', async () => {
			...
			for (const length of lengths) {
				...
				expect((result.content[0] as any).data.length).toBe(expectedWordCount);
			}
		});
		...
		it('should handle generation failures', async () => {
			...
			jest.mocked(LangChainCompilationService).mockImplementationOnce(
				() =>
					({
						initialize: jest.fn(),
						generateMarketingMaterials: jest
							.fn()
							.mockRejectedValue(new Error('Generation failed') as any),
					}) as any
			);
	...
	describe('buildVectorStoreHandler', () => {
		it('should build vector store successfully', async () => {
			...
			expect((result.content[0] as any).data.enhanced).toBe(true);
			expect((result.content[0] as any).data.vectorIndexed).toBe(true);
			expect((result.content[0] as any).data.documentsIndexed).toBe(2);
		});
		...
		it('should handle vector store build failures', async () => {
			const { LangChainHMSVectorStore } =
				await import('../../../src/services/ai/hms-vector-store.js');
			jest.mocked(LangChainHMSVectorStore.fromDocuments).mockRejectedValueOnce(
				new Error('Vector store init failed')
			);
		...
		it('should include document metadata in vector store', async () => {
			const { LangChainHMSVectorStore } =
				await import('../../../src/services/ai/hms-vector-store.js');
			const mockFromDocuments = jest.mocked(LangChainHMSVectorStore.fromDocuments);
			mockFromDocuments.mockResolvedValueOnce({} as any);
	...
	describe('Handler Input Validation', () => {
		...
		it('should require materialType for marketing generation', () => {
			expect(generateMarketingMaterialsHandler.inputSchema.required).toContain(
				'materialType'
			);
		});
	...
	describe('Continuous Learning Integration', () => {
		it('should collect feedback for successful operations', async () => {
			const { LangChainContinuousLearningHandler } =
				await import('../../../src/handlers/langchain-continuous-learning-handler.js');
