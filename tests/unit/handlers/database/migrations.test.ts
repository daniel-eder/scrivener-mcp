...
jest.mock('../../../../src/utils/database.js', () => ({
	toDatabaseError: jest.fn((error, operation) => {
		...
		return new AppError(`Database error during ${operation}`, ErrorCode.DATABASE_ERROR, {
			operation,
			originalError: error,
		});
	}),
...
describe('MigrationManager', () => {
	...
	describe('migrate', () => {
		...
		it('should run pending migrations', async () => {
			const mockMigration: Migration = {
				version: 2,
				name: 'test_migration',
				up: jest.fn<Migration['up']>().mockResolvedValue(undefined),
			};

			// Override migrations for testing
			migrationManager['migrations'] = [
				{ version: 1, name: 'completed', up: jest.fn<Migration['up']>() },
				mockMigration,
			];
		...
		it('should skip if no pending migrations', async () => {
			mockSQLiteManager.queryOne.mockReturnValue({ version: 10 });
		...
		it('should rollback on migration failure', async () => {
			const failingMigration: Migration = {
				...
				up: jest.fn<Migration['up']>().mockRejectedValue(new Error('Migration failed')),
			};
			...
			expect(toDatabaseError).toHaveBeenCalledWith(expect.any(Error), 'migration 2');
		});
	...
	describe('rollbackTo', () => {
		...
		it('should rollback migrations with down methods', async () => {
			const migration: Migration = {
				version: 2,
				name: 'rollback_test',
				up: jest.fn<Migration['up']>(),
				down: jest.fn<NonNullable<Migration['down']>>().mockResolvedValue(undefined),
			};
	...
	describe('getStatus', () => {
		it('should return correct migration status', () => {
			...
			migrationManager['migrations'] = [
				{ version: 1, name: 'first', up: jest.fn<Migration['up']>() },
				{ version: 2, name: 'second', up: jest.fn<Migration['up']>() },
				{ version: 3, name: 'third', up: jest.fn<Migration['up']>() },
			];
