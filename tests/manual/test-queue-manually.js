...
async function testQueue() {
	...
	try {
		// Create JobQueueService for the test project directory (embedded queue only)
		const jobQueue = new JobQueueService(testDir);
		...
		const jobId = await jobQueue.addJob(JobType.ANALYZE_DOCUMENT, {
			documentId: 'test-doc',
		...
		console.log(
			`   ✓ Queue stats: waiting=${stats.waiting}, active=${stats.active}, completed=${stats.completed}\n`
		);
