...
- **JS fallback vector engine**: semantic search, analogies, and dream mode work without `@writerslogic/hms-native`. Uses TF-IDF random-projection vectors with FNV-1a hashing, stopword filtering, log-normalized TF, k-means++ clustering. Native Rust engine is used transparently when available.
