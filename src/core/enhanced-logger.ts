...
export class EnhancedLogger extends EventEmitter {
	...
	private metricsIntervals: ReturnType<typeof setInterval>[] = [];
	...
	async close(): Promise<void> {
		...
		for (const interval of this.metricsIntervals) {
	...
	private createWinstonLogger(serviceName: string): winston.Logger {
		...
		return winston.createLogger({
			...
			exceptionHandlers: [
				new winston.transports.File({
					filename:
						this.config.filePath?.replace(/\.log$/, '.exceptions.log') ||
						'./logs/exceptions.log',
				}),
			],
			rejectionHandlers: [
				new winston.transports.File({
					filename:
						this.config.filePath?.replace(/\.log$/, '.rejections.log') ||
						'./logs/rejections.log',
				}),
			],
		});
			...
			case 'pretty':
				formats.push(
					...
					winston.format.printf((info: winston.Logform.TransformableInfo) => {
						const context = info.correlationId
							? ` [${String(info.correlationId).substring(0, 8)}]`
							: '';
