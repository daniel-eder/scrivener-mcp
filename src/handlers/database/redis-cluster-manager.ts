...
export class RedisClusterManager extends EventEmitter {
	...
	async invalidateByTags(tags: string[]): Promise<number> {
		...
		try {
			// Collect all keys from all tags in parallel
			const tagKeys = tags.map((tag) => `tag:${tag}`);
			const memberResults = await Promise.all(
				tagKeys.map((tagKey) => this.cluster!.smembers(tagKey))
			);

			// Gather all keys to delete (cache entries + tag index keys)
			const allKeysToDelete: string[] = [...tagKeys];
			for (const keys of memberResults) {
				if (keys.length > 0) {
			...
			if (allKeysToDelete.length > 0) {
				totalDeleted = await this.cluster.del(...allKeysToDelete);
			}
	 ...
	 */
	async pipeline(
		operations: Array<{ op: string; key: string; value?: unknown; ttl?: number }>
	): Promise<unknown[]> {
		if (!this.cluster) throw new AppError('Cluster not initialized', ErrorCode.INVALID_STATE);
		...
		try {
					...
					case 'set':
						if (operation.ttl) {
							pipeline.setex(
								operation.key,
								operation.ttl,
								this.serialize(operation.value)
							);
						} else {
			...
			logger.error('Pipeline operations failed', {
	...
	async getHealth(): Promise<ClusterHealth> {
		...
		try {
			...
			const healthyNodes = nodeLines.filter((line: string) =>
		...
		try {
			...
			if (memoryInfo.percentage > 80) {
				...
				result.recommendations.push(
					'Memory usage is high (>80%), consider increasing cluster capacity'
				);
			...
			if (memoryInfo.percentage > 90) {
				result.recommendations.push(
					'Critical memory usage (>90%), immediate action required'
				);
			}
