...
export class WriteThroughStrategy implements CacheStrategy {
	...
	async execute<T>(
		...
		await Promise.all([this.cacheManager.set(key, value, options), this.writeFn(key, value)]);
...
export class MultiLevelStrategy implements CacheStrategy {
	...
	async execute<T>(
		...
		await Promise.all([this.setL1(key, value), this.cacheManager.set(key, value, options)]);
