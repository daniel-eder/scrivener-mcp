...
1. **At startup:** the `project` skill (`open_project`, `get_structure`, `refresh_project`, `close_project`, `discover_projects`, `detect_open_project`, `verify_project_integrity`, `get_compile_settings`, `get_manuscript_briefing`, `list_snapshots`, `read_snapshot`, `compare_snapshot`, `create_snapshot`) plus `list_skills` and `use_skill`.
